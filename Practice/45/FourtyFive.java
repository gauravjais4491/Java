...
class FourtyFive {
    public static void main(String[] shruti) throws Exception {
        ...
        Connection con = null;
        try {
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306/KE021", "root", "shruti@24906");
            ...
            if (s_balance < amount)
                ...
                con.setAutoCommit(false); // Set the connection property disabled because query is not completely
                                          // executed.
                Statement stmt = con.createStatement();
                s_balance -= amount;
                r_balance += amount;
                stmt.addBatch("update account set balance = " + s_balance + " where account_number = " + sender);
                stmt.addBatch("upda account set balance = " + r_balance + " where account_number = " + receiver);
                /*
                 * pst = con.
